...
class DEX_YCB(torch.utils.data.Dataset):
    ...
    def load_data(self):
        db = COCO(osp.join(self.annot_path,
                  "DEX_YCB_s0_{}_data.json".format(self.data_split)))
        ...
        for aid in db.anns.keys():
            ...
            if self.data_split == 'train':
                joints_coord_cam = np.array(
                    ann['joints_coord_cam'], dtype=np.float32)  # meter
                cam_param = {k: np.array(v, dtype=np.float32)
                             for k, v in ann['cam_param'].items()}
                joints_coord_img = np.array(
                    ann['joints_img'], dtype=np.float32)
                hand_type = ann['hand_type']

                bbox = get_bbox(joints_coord_img[:, :2], np.ones_like(
                    joints_coord_img[:, 0]), expansion_factor=1.5)
                bbox = process_bbox(
                    bbox, img['width'], img['height'], expansion_factor=1.0)

                if bbox is None:
                    continue

                mano_pose = np.array(
                    ann['mano_param']['pose'], dtype=np.float32)
                mano_shape = np.array(
                    ann['mano_param']['shape'], dtype=np.float32)
                ...
                joints_coord_cam = np.array(
                    ann['joints_coord_cam'], dtype=np.float32)
                ...
                if bbox is None:
                    bbox = np.array(
                        [0, 0, img['width']-1, img['height']-1], dtype=np.float32)
    ...
    def __getitem__(self, idx):
        ...
        if self.data_split == 'train':
            ...
            joints_img = np.dot(img2bb_trans, joints_img_xy1.transpose(
                1, 0)).transpose(1, 0)[:, :2]
            # normalize to [0,1]
            ...
            root_joint_cam = copy.deepcopy(
                joints_coord_cam[self.root_joint_idx])
            # root-relative
            ...
            joints_coord_cam = np.dot(
                rot_aug_mat, joints_coord_cam.transpose(1, 0)).transpose(1, 0)
            ...
            inputs = {'img': img}
            targets = {'joints_img': joints_img, 'joints_coord_cam': joints_coord_cam,
    ...
    def evaluate(self, outs, cur_sample_idx):
        ...
        for n in range(sample_num):
            ...
            self.eval_result[0].append(
                np.sqrt(np.sum((joints_out - joints_gt)**2, 1)).mean())
